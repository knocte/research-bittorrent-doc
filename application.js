...
(function (root, factory) {
  ...
  Lightbox.prototype.changeImage = function(imageNumber) {
    ...
    var isCurrentVideo = $image[0].src.endsWith(".mp4");
    var isNextVideo = this.album[imageNumber].link.endsWith(".mp4");
    ...
    var handleLoaded = function() {            
      ...
      if( !isNextVideo && isCurrentVideo )
      {
        console.log("replacing with image");
        $('.lb-image').replaceWith("<img class=\"lb-image\"/>");
      }
      else if( isNextVideo && !isCurrentVideo )
      {
        $('.lb-image').replaceWith("<video class=\"lb-image\" controls></video>");
      }
      ...
      $preloader = $(preloader);
      pwidth = isNextVideo ? preloader.videoWidth : preloader.width;
      pheight = isNextVideo ? preloader.videoHeight : preloader.height;

      $image.width(pwidth);
      $image.height(pheight);

      console.log( pwidth, $image.width() );
      ...
      if (self.options.fitImagesInViewport) {
        ...
        if ((pwidth > maxImageWidth) || (pheight > maxImageHeight)) {
          if ((pwidth / maxImageWidth) > (pheight / maxImageHeight)) {
            imageWidth  = maxImageWidth;
            imageHeight = parseInt(pheight / (pwidth / imageWidth), 10);
            $image.width(imageWidth);
            $image.height(imageHeight);
          } else {
            imageHeight = maxImageHeight;
            imageWidth = parseInt(pwidth / (pheight / imageHeight), 10);
            $image.width(imageWidth);
            $image.height(imageHeight);
          }
        }
    ...
    else
    {
      ...
      preloader.src = this.album[imageNumber].link;
      preloader.addEventListener('loadeddata', handleLoaded);      
      preloader.load();
    }
